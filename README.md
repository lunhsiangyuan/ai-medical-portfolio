...
導入 Claude Code 後，搭配 Agent Teams 建構多 Agent 協作系統與完整開發架構。在一場 [special talk](slides/ai-agent-research-talk-final.pptx) 中展示成果：一位研究者搭配 10 個 AI Agent，產出等同完整研究團隊。同時擴展至臨床工具、教學自動化、商業營運、美國醫療教育財務法規研究等各個面向。
...
| 審稿輔助系統 | 期刊論文四維度系統性評估，一年半內完成 60–70+ 篇審稿 | O-1A 等級審稿量 |
...
自建的 AI Agent 管理平台，部署於 Mac Mini 24/7 運行。這是 AI 工作力的未來趨勢：為每位白領工作者部署一個 24/7 的 AI 員工——處理文書、回覆訊息、整理資料、追蹤進度。想像為飛天學院的每位教職員配備一個專屬 AI 助手，全年無休。
...
### 🔄 Agentic AI 趨勢：自主閉環工作流

> AI 產業正從「人下指令 → AI 回答」轉向「AI 自主規劃 → 執行 → 驗證 → 修復」的 Agentic 模式。
> 我已在生產環境中實現這個趨勢：
...
│                  └──────────┘                        │
│                                                      │
│  安全機制：Circuit Breaker — 連續失敗 3 次自動停機    │
└─────────────────────────────────────────────────────┘
```

- **LongChain** — 複雜任務自動拆解為結構化計畫，逐步執行並驗證
- **Ralph Loop** — 閉環迭代：失敗→分析原因→自動修復→重新驗證，直到完成
- **Auto-Verify** — 驗證分級（自動測試 > 腳本檢查 > AI 評估），品質客觀可控
- **實際應用** — 過夜自動產出教材、批次資料處理、全自動程式開發與測試

從「人盯著 AI 做事」進化為「AI 自主完成，人只做最終確認」
